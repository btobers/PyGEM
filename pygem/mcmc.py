...
def log_normal_density(x, **kwargs):
    ...
    Parameters:
    - x: input data point or batch of data points.
...
def plot_chain(m_primes, m_chain, mb_obs, ar, title, ms=1, fontsize=8, show=False, fpath=None):
    ...
    axes[0].plot([],[],label=f'median={np.median(m_chain[:, 0]):.3f}\niqr={np.subtract(*np.percentile(m_chain[:, 0], [75, 25])):.3f}')
    ...
    axes[1].plot([],[],label=f'median={np.median(m_chain[:, 1]):.3f}\niqr={np.subtract(*np.percentile(m_chain[:, 1], [75, 25])):.3f}')
    ...
    axes[2].plot([],[],label=f'median={np.median(m_chain[:, 2]):.3f}\niqr={np.subtract(*np.percentile(m_chain[:, 2], [75, 25])):.3f}')
    ...
    axes[-2].fill_between(np.arange(len(ar)),mb_obs[0]-(2*mb_obs[1]),mb_obs[0]+(2*mb_obs[1]),color='grey',alpha=.3)
    axes[-2].fill_between(np.arange(len(ar)),mb_obs[0]-mb_obs[1],mb_obs[0]+mb_obs[1],color='grey',alpha=.3)
    axes[-2].plot(m_primes[:, -1],'.',ms=ms, c='tab:blue')
    axes[-2].plot(m_chain[:, -1],'.',ms=ms, c='tab:orange')
    axes[-2].plot([],[],label=f'median={np.median(m_chain[:, -1]):.3f}\niqr={np.subtract(*np.percentile(m_chain[:, -1], [75, 25])):.3f}')
    ...
    axes[-1].plot(np.convolve(ar, np.ones(100)/100, mode='valid'), 'k', label='moving avg.', lw=1)
    ln1 = axes[-1].legend(loc='upper left',handlelength=.5, borderaxespad=0, fontsize=fontsize)
    legs.append(ln1)
